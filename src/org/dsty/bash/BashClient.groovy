...
class BashClient implements Serializable {
    ...
    String formatScript(String userScript, Boolean consoleOutput=true, Boolean failFast=true) {
        String showOutput = 'exec 2> >(tee -a stderr stdall) 1> >(tee -a stdout stdall)'
        String hideOutput = 'exec 3>/dev/null 2> >(tee -a stderr stdall >&3) 1> >(tee -a stdout stdall >&3)'
        String exec = consoleOutput ? showOutput : hideOutput
    ...
    private List<String> readOutputs() {
        ...
        List results = [stdOut, stdErr, output].collect { result ->
            result.replaceAll('(?m)^\\+.*(?:\r?\n)?', '').trim()
        }
